...
//Definition: ENCODER
Encoder encoder[4] = {Encoder(8,1), Encoder(9,1), Encoder(9,2), Encoder(8,1)};
...
#define MULTIPLIERDIF 0.33
#define MULTIPLIERSPE 0.77
/*****************************************************
...
void Pilot::update() {
  ...
  for(int i=0; i<4; i++){
    currentspeed = map((encoder[i].getSpeed() - 127),-127,127,-255,255);
      power = MULTIPLIERDIF * ( _wantedSpeed[i] - currentspeed ) + MULTIPLIERSPE * _wantedSpeed[i];
      steerMotor(i, power);
  }  
}
